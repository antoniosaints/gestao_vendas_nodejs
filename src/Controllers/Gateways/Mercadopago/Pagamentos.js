...
const createPayment = async (req, res) => {
  const client = new MercadoPagoConfig({ accessToken: process.env.APIMERCADOPAGO });
...
const getPayment = async (req, res) => {
    ...
    payment.get({
        id: idPayment,
    }).then(({id, status, date_approved, date_created, date_last_updated, date_of_expiration, payment_method, point_of_interaction}) => {
        const returnedData = {
            id,
            status,
            date_approved,
...
export { createPayment, getPayment, cancelPayment, refoundPayment };